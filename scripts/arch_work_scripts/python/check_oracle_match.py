...
# -*- coding: utf-8 -*-
#
# check_oracle_match.py: Oracle monitoring - Match status keywords
        ...
        for instance_name in instances:
            ...
            if not data:
                ...
                if opt_multiline:
                    if int(data[0]) == 0:
                        if not state[1]:
                            result['ok'].append('OK: %s - No value' % instance_name)
                        else:
                            result['ok'].append('OK: %s - %s' % (instance_name, state[1]))
                        continue

                # Data is multi-lines if Table == True in SNMPd's plugin check-oracle.py
                for index, line in enumerate(data):

                    # Ignore data[0] if opt_multiline is set because first tuple contains count
                    if opt_multiline and index == 0:
                        continue

                    if re.search('error|critical', line, re.I):
                        result['critical'].append('CRITICAL: %s - %s' % (instance_name, line))
    
                    elif re.search('warning', line, re.I):
                        result['warning'].append('WARNING: %s - %s' % (instance_name, line))
    
                    elif re.search('ok', line, re.I):
                        result['ok'].append('OK: %s - %s' % (instance_name, line))
    
                    else:
                        result['unknown'].append('UNKNOWN: %s - %s' % (instance_name, line))
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.gnu_getopt(sys.argv[1:], 'B:c:C:hH:N:w:x:m')
