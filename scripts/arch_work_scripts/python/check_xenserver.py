#!/usr/bin/env python
# $Rev: 1229 $:  Revision of last commit
# $Author: a.lamjarhjarh $:  Author of last commit
# $Date: 2015-10-21 11:16:16 +0200 (mer. 21 oct. 2015) $:  Date of last commit
# $HeadURL: http://svn.e-merchant.net/svn/norel-puppet-cms/modules/nagios/files/default/scripts/check_xenserver.py $: Path 
#
# Check XenServer
...
# - v1.3: Julien CIVRAY : Modification pour les SR de type NFS et EXT 
# 
...
def sr(session, sr_name, warning, critical, performancedata_format):
    ...
    if sr:
        ...
        '''print "sr_phys_util",sr_phys_util'''
        sr_virtual_alloc = session.xenapi.SR.get_virtual_allocation(sr[0])
        '''print "sr_virtual_alloc",sr_virtual_alloc'''
...
def check_sr(session, args):
    ...
    for cur_sr in sr_list:
        ...
        if sr_name not in exclude_srs:
            ...
            if session.xenapi.SR.get_type(cur_sr) == 'ext' or session.xenapi.SR.get_type(cur_sr) == 'nfs' :
...
def check_hosts(session, args):
    ...
    if len(live_hosts) == 0:
        status = "Critical"
        msg = "All hosts are dead ", dead_hosts,
        exit = 2
    elif len(dead_hosts) >= 1:
        status = "Warning"
        msg = "Dead hosts", dead_hosts,
        exit = 1
    else:
        status = "OK"
        msg = "All Hosts are UP ", live_hosts,
        exit = 0
