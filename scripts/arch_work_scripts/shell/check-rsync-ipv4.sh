...
# vim: ts=4 sw=4 et
#
# $HeadURL: http://svn.e-merchant.net/svn/norel-puppet/modules/snmp/files/plugins/check-rsync-ipv4.sh $


BASEOID='.1.3.6.1.4.1.38673.1.32'
MODE=''
...
# Function : _rsync_status
_rsync_status() {
    [ ! -d "${TMPDIR}" ] && mkdir "${TMPDIR}"

    ipv4_check="rsync $(ip addr show dev eth0 | awk '$1=="inet" {sub("/[0-9]+","",$2);print $2}')::"

    eval "${ipv4_check}" >${TMPDIR}/result_msg 2>&1
