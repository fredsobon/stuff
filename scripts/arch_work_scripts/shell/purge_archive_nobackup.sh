...
ORACLE_SID=`cat /etc/oratab  | grep -iv "#" | grep -iw $1 | cut -d: -f1`
ORACLE_HOME=`cat /etc/oratab | grep -iv "#" | grep -iw $1 | cut -d: -f2`
TNS_ADMIN=$ORACLE_HOME/network/admin
RETENTION=$2

export ORACLE_SID=oraref1p
export ORACLE_HOME
...
affiche_param_global()
{
  ...
  echo -e "GLOBAL_PARAMETERS -email_list toto@titi.fr \n"
}
...
pb_exit1()
{ 
  send_email "Probleme lors de la purge " "$1"
  exit 1
}
#
#--------------------------------------------------------------------------------------------
